...
int main() {
    ...
    for (int i = 0; i < NUM_ENTREGADORES; i++) {
        int* id = malloc(sizeof(int)); // Aloca memória para o ID do entregador
        *id = i;
        
        if (i % 2 == 0) 
            pthread_create(&entregadores[i], NULL, entregador_veterano, id);
        else 
            pthread_create(&entregadores[i], NULL, entregador_novato, id);
    }
...
void* entregador_veterano(void* arg) {
    ...
    while (1) { 
        ...
        if (pthread_mutex_trylock(&pedidos[restaurante]) == 0) { // Tenta pegar o lanche
            ...
            sleep(1); // Pequena pausa antes de pegar o próximo pedido
